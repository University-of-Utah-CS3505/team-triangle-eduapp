...
gameplay::gameplay(engine& eng)
    ...
    : _editor{15, 800, 650}, _engine{eng}, _level(0),
      _text_handle(_engine.add_event_listener(
              ...
              [this](auto e) {
                _input_con.handleEvents(_text_doc, _text_view, _engine.window(), e);
                return true;
})),

      _pressed_handle(_engine.add_event_listener(
              sf::Event::KeyPressed,
              [this](auto e) { if(_handle_keyboard(e))
                        { return true;
                    } else{
                     _input_con.handleEvents(_text_doc, _text_view, _engine.window(), e);
                     return true;
      ...
      _released_handle(_engine.add_event_listener(
                           ...
                           [this](auto e){_input_con.handleEvents(_text_doc, _text_view, _engine.window(), e);
                return true;})),

      _mouse_click(_engine.add_event_listener(
              sf::Event::MouseButtonPressed,
                        [this](auto e) { if(handle_mouse(e)){
                    return true;
                   } else{
        _input_con.handleEvents(_text_doc, _text_view, _engine.window(), e);
                     }})),
    ...
    _text_view(_engine.window(), "./../team-triangle-eduapp/assets/")
    {
    _load_level(1);
...
std::unique_ptr<game_state> gameplay::update() {
   ...
   // _engine.window().draw(_editor);
    ...
    _editor_holder.setPosition(400,10);
    //_engine.window().draw(_editor_holder);
    _engine.window().setView(_text_view.getCameraView());
    _text_view.draw(_engine.window(), _text_doc);
    return nullptr;
}
