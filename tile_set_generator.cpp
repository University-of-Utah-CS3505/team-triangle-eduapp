...
boost::multi_array<int,2> tile_set_generator::tile_set_generate_level(int level) {
    ...
    _levels_boost = boost::extents[size][size];
    for (pt::ptree::value_type& row : root.get_child("tiles")) {
        ...
        for (pt::ptree::value_type& tile : row.second) {
            std::cout<< "coor: " << x << " " << y << " -- val: "
                      ...
                      << std::endl;
            _levels_boost[x][y] = tile.second.get_value<int>();
            y++;
        }
        x++;
    }
...
sf::Texture tile_set_generator::generate_texture(boost::multi_array<int,2> boost_array){
