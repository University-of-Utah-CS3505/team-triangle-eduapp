...
void TextView::draw(sf::RenderTarget& window, TextDocument& document) {
    ...
    for (int lineNumber = 1; lineNumber <= document.getLineCount();
         lineNumber++) {
        int lineHeight = 1;
        ...
        lineNumberText.setPosition(-this->marginXOffset,
                                   blockHeight * (lineNumber - 1));

        sf::RectangleShape marginRect(
                sf::Vector2f(this->marginXOffset - 5, blockHeight));
        marginRect.setFillColor(this->colorMargin);
        marginRect.setPosition(-this->marginXOffset,
                               blockHeight * (lineNumber - 1));
...
TextView::DocCoords TextView::getDocumentCoords(float mouseX,
                                                float mouseY,
...
// TODO: Agregar parametros para saber si tengo que agregar otro, actualizar
// selecciones o lo que sea
// TODO: Esta funcion solo sirve para la ultima seleccion, manejarlo por
// parametros??
void TextView::cursorActive(float mouseX,
                            float mouseY,
                            const TextDocument& document) {
    TextView::DocCoords docCoords =
            this->getDocumentCoords(mouseX, mouseY, document);
...
void TextView::swapCursorLine(TextDocument& document, bool swapWithUp) {
    ...
    if (swapWithUp) {
        ...
        document.swapLines(
                currentLine,
                std::min(currentLine + 1, document.getLineCount() - 1));
    }
}

// Una seleccion inicial selecciona el propio caracter en el que estoy
void TextView::startSelectionFromMouse(float mouseX,
                                       float mouseY,
                                       const TextDocument& document) {
    TextView::DocCoords docCoords =
            this->getDocumentCoords(mouseX, mouseY, document);
...
void TextView::startSelectionFromCursor() {
    this->content.removeSelections();
    this->content.createNewSelection(this->cursor.getLineN(),
                                     this->cursor.getCharN());
}
...
bool TextView::deleteSelections(TextDocument& document) {
    ...
    // Tomar el inicio de lastSelection, calcular el largo y borrar desde el
    // inicio,
    if (lastSelection.activa) {
        int startLineN = SelectionData::getStartLineN(lastSelection);
        int startCharN = SelectionData::getStartCharN(lastSelection);
        ...
        int amount = document.charAmountContained(
                             startLineN, startCharN, endLineN, endCharN) -
                     1;
...
void TextView::moveCursorRight(const TextDocument& document,
    ...
    if (this->cursor.getCharN() >= charsInLine) {
        int newCursorLine =
                std::min(this->cursor.getLineN() + 1, document.getLineCount());
...
void TextView::moveCursorUp(const TextDocument& document,
                            bool updateActiveSelections) {
    if (this->cursor.getLineN() > 0) {
        int charsInPreviousLine =
                document.charsInLine(this->cursor.getLineN() - 1);
        int currentCharPos = this->cursor.getCharN();

        // Si el caracter actual existe en la linea de arriba, voy solo arriba,
        // sino voy al final de la linea de arriba
        if (currentCharPos <= charsInPreviousLine &&
            this->cursor.getMaxCharNReached() <= charsInPreviousLine) {
            this->cursor.moveUpToMaxCharN();
        } else {
            this->cursor.setPosition(this->cursor.getLineN() - 1,
                                     charsInPreviousLine);
        }
    }
...
void TextView::moveCursorDown(const TextDocument& document,
    ...
    if (this->cursor.getLineN() < document.getLineCount() - 1) {
        ...
        if (currentCharPos <= charsInNextLine &&
            this->cursor.getMaxCharNReached() <= charsInNextLine) {
...
void TextView::moveCursorToEnd(const TextDocument& document,
                               bool updateActiveSelections) {
    int charsInLine = document.charsInLine(this->cursor.getLineN());
...
void TextView::moveCursorToStart(const TextDocument& document,
                                 bool updateActiveSelections) {
...
void TextView::handleSelectionOnCursorMovement(bool updateActiveSelections) {
    if (updateActiveSelections) {
        this->content.updateLastSelection(this->cursor.getLineN(),
                                          this->cursor.getCharN());
    } else {
        this->content.removeSelections();
    }
...
void TextView::removeSelections() { this->content.removeSelections(); }
...
void TextView::rotateLeft() { this->camera.rotate(this->deltaRotation); }

void TextView::rotateRight() { this->camera.rotate(-this->deltaRotation); }

void TextView::zoomIn() { this->camera.zoom(this->deltaZoomIn); }

void TextView::zoomOut() { this->camera.zoom(this->deltaZoomOut); }
